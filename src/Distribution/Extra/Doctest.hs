...
generateBuildModule testSuiteName flags pkg lbi = do
  ...
  withTestLBI pkg lbi $ \suite suitecfg -> when (testName suite == fromString testSuiteName) $ do
    ...
    let appendBuildDoctests withCompLBI mbCompName compExposedModules compBuildInfo =
         withCompLBI pkg lbi $ \comp compCfg -> do
           ...
           let compSuffix          = maybe "" ("_" ++) (mbCompName comp)
           ...
           appendFile buildDoctestsFile $ unlines
             ...
             , module_sources_comp ++ " = " ++ show (map display module_sources ++ additionalModules)
             , ""
             ]

    -- For now, we only check for doctests in libraries and executables.
    appendBuildDoctests withLibLBI mbLibraryName           exposedModules libBuildInfo
    appendBuildDoctests withExeLBI (Just . executableName) exeMainIs      buildInfo
  where
    ...
    exeMainIs :: Executable -> [ModuleName]
    exeMainIs exe = [ ModuleName.fromString
                    $ dropExtension -- Be careful: the IsString instance doesn't
                                    -- like "Main.hs", only "Main"
                    $ modulePath exe
                    ]
